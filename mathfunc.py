...
def gcd(message):
    ...
    for_msg = f'MDC dos números {numbers}'
    
    for i in numbers:
        if len(numbers) == 1:
            c = numbers[0]
            message_gcd = f'MDC do número {c} é {c}.'
        else:
            while len(numbers) != 1:
                a = numbers[0]
                b = numbers[1]
...
def lcm(message):
    ...
    numbers = [int(x) for x in message.split(' ')[1:]]
    for_msg = f'MMC  {numbers}'
    
    for i in numbers:
        if len(numbers) == 1:
            c = numbers[0]
            message_lcm = f'MMC do número {c} é {c}.'
        else:
            while len(numbers) != 1:
                a = numbers[0]
                b = numbers[1]
...
def factor(message):
    ...
    if len(factor) <= 1:
        message_factor = f'{for_msg} - número primo.'
    else:
        message_factor = f'Fatoração de número {for_msg} = {factor}.'
...
def polynoms_add(message):
    ...
    polynoms = message.split('/polinomio_add ')[1]
    ...
    msg = f'Soma de polinômios com coeficientes {coef_1} e {coef_2}'
    ...
    message_poly_add = f'{msg} é polinomial com coeficientes {sum_coef}.'
...
def polynoms_sub(message):
    ...
    polynoms = message.split('/polinomio_sub ')[1]
    poly_1, poly_2 = polynoms.split(')-(')[0:2]
    coef_1 = [int(x) for x in re.findall(r"[*-][0-9]+|[0-9]+", poly_1)]
    coef_2 = [int(x) for x in re.findall(r"[*-][0-9]+|[0-9]+", poly_2)]
    msg = f'Subtração de polinômios com coeficientes {coef_1} e {coef_2}'
    ...
    message_poly_sub = f'{msg} é polinômio com coeficientes {sub_coef}.'
...
def polynoms_mul(message):
    ...
    polynoms = message.split('/polinomio_mul ')[1]
    ...
    for_msg = f'Multiplicação de polinômios com coeficientes {coef_1} e {coef_2}'
    ...
    msg_poly_mul = f'{for_msg} é polinômio com coeficientes {coef_mul}.'
...
def polynoms_div(message):
    ...
    polynoms = message.split('/polinomio_div ')[1]
    polynom_1, polynom_2 = polynoms.split('/')[0:2]
    coef_1 = [int(x) for x in re.findall(r"[*-][0-9]+|[0-9]+", polynom_1)]
    coef_2 = [int(x) for x in re.findall(r"[*-][0-9]+|[0-9]+", polynom_2)]
    for_msg = f'Divisão de polinômios com coeficientes {coef_1} e {coef_2}'
    
    if len(coef_2) > len(coef_1):
        msg_poly_div = f'{for_msg} é impossível, porque a potência do segundo polinômio é maior.'
    else:    
        p = 0
        ...
        if coef_1 == [0 for i in range(len(coef_1))]:
            ...
            if len(rem) == 1:
                rm = f'o resto é {rem[0]}.'
            else:
                rm = f'o resto é {rem}.'
    
        msg_poly_div = f'{for_msg} é polinômio com coeficientes {cd} e {rm}'
...
def lin_dep(message):
    ...
    for_msg = f'Decomposição linear do número {numbers[0]} e {numbers[1]}'
    ...
    msg_lin_dep = f'{for_msg} é {coefs} = {check_gcd}.'
